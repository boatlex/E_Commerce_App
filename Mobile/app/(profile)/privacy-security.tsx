import { View, Text, TouchableOpacity, ScrollView, Switch } from 'react-native'
import React, { useState } from 'react'
import SafeScreen from '../components/SafeScreen'
import { router } from 'expo-router'
import { Ionicons } from '@expo/vector-icons'


type SecurityOption = {
    id: string
    icon: string
    title: string
    description: string
    type: "navigation" | "toggle"
    value?: boolean
}
const PrivacyAndSecurityScreen = () => {

    const [twoFactorEnabled, setTwoFactorEnabled] = useState(false)
    const [biometricEnabled, setBiometricEnabled] = useState(true)
    const [pushNotifications, setPushNotification] = useState(true)
    const [emailNotifications, setEmailNotifications] = useState(true)
    const [marketEmails, setMarketEmails] = useState(false)
    const [shareData, setShareData] = useState(false)


    const SecuritySettings: SecurityOption[] = [
        {
            id: "password",
            icon: "lock-closed-outline",
            title: "Change Password",
            description: "Update your account password",
            type: "navigation",
        },
        {
            id: "two-factor",
            icon: "shield-checkmark-outline",
            title: "Two-Factor Authentification",
            description: "Add an extra layer of security",
            type: "toggle",
            value: twoFactorEnabled
        },
        {
            id: "biometric",
            icon: "finger-print-outline",
            title: "Biometric Login",
            description: "Use face Id or touch Id",
            type: "toggle",
            value: biometricEnabled
        },

    ]
    const PrivacySettings: SecurityOption[] = [
        {
            id: "email",
            icon: "mail-outline",
            title: "Email Notifications",
            description: "Receive push notifications",
            type: "toggle",
            value: emailNotifications
        },
        {
            id: "push",
            icon: "notifications-outline",
            title: "Push Notifications",
            description: "Receive push notifications",
            type: "toggle",
            value: pushNotifications
        },
        {
            id: "marketing",
            icon: "megaphone-outline",
            title: "Marketing Emails",
            description: "Receive Promotional Emails",
            type: "toggle",
            value: marketEmails
        },
        {
            id: "data",
            icon: "analytics-outline",
            title: "Share Usage Data",
            description: "Help us improve the app",
            type: "toggle",
            value: shareData
        },
    ]

    const accountSettings = [
        {
            id: "activity",
            icon: "time-outline",
            title: "Account Activity",
            description: "View recent activity",


        },
        {
            id: "data-download",
            icon: "download-outline",
            title: "Download Your Data",
            description: "Get a copy of your data",


        },
        {
            id: "device",
            icon: "phone-portrait-outline",
            title: "Connected Devices",
            description: "Manage device with access",


        },

    ]

    const handleToggle = (id: string, value: boolean) => {
        switch (id) {
            case "two-factor":
                setTwoFactorEnabled(value)
                break;
            case "biometric":
                setBiometricEnabled(value)
                break;
            case "push":
                setPushNotification(value)
                break;
            case "email":
                setEmailNotifications(value)
                break;
            case "marketing":
                setMarketEmails(value)
                break;
            case "data":
                setShareData(value)
                break;
        }
    }

    return (
        <SafeScreen>
            {/* Header */}
            <View className='px-6 pb-5 pt-2 border-b border-surface flex-row items-center'>
                <TouchableOpacity
                    className='mr-4'
                    onPress={() => router.back()}
                >
                    <Ionicons name='arrow-back' size={28} color={"#fff"} />
                </TouchableOpacity>
                <Text className='text-text-primary font-bold text-2xl'>
                    Privacy and Security
                </Text>
            </View>

            <ScrollView
                className='flex-1'
                showsVerticalScrollIndicator={false}
                contentContainerStyle={{ paddingBottom: 50 }}
            >
                {/* Security settings */}

                <View className='px-6 pt-6'>
                    <Text className='text-text-primary font-bold text-lg mb-4'>Security</Text>
                    {SecuritySettings.map((setting) => (
                        <TouchableOpacity
                            key={setting.id}
                            className=' bg-surface rounded-2xl mb-3 p-4'
                            activeOpacity={setting.type === "toggle" ? 1 : 0.7}
                        >
                            <View className='flex-row items-center'>
                                <View className='bg-primary/20 rounded-full justify-center
                  w-12 h-12 items-center mr-4'>
                                    <Ionicons name={setting.icon as any} size={24} color={"#1DB954"} />
                                </View>
                                <View className='flex-1'>
                                    <Text
                                        className='text-text-primary text-base mb-1 font-bold'
                                    >{setting.title}</Text>
                                    <Text className='text-text-secondary text-sm '>{setting.description}</Text>
                                </View>
                                {setting.type === "toggle" ? (
                                    <Switch value={setting.value}
                                        onValueChange={(value) => handleToggle(setting.id, value)}
                                        trackColor={{ false: "#2A2A2A", true: "#1DB954" }}
                                        thumbColor={"#FFFFFF"}
                                    />
                                ) : (
                                    <Ionicons name='chevron-forward' size={20} color={"#666"} />
                                )}
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>

                {/* Privacy Settings */}
                <View className='px-6 pt-6'>
                    <Text className='text-text-primary font-bold text-lg mb-4'>Privacy</Text>
                    {PrivacySettings.map((setting) => (
                        <TouchableOpacity
                            key={setting.id}
                            className=' bg-surface rounded-2xl mb-3 p-4'
                            activeOpacity={setting.type === "toggle" ? 1 : 0.7}
                        >
                            <View className='flex-row items-center'>
                                <View className='bg-primary/20 rounded-full justify-center
                  w-12 h-12 items-center mr-4'>
                                    <Ionicons name={setting.icon as any} size={24} color={"#1DB954"} />
                                </View>
                                <View className='flex-1'>
                                    <Text
                                        className='text-text-primary text-base mb-1 font-bold'
                                    >{setting.title}</Text>
                                    <Text className='text-text-secondary text-sm '>{setting.description}</Text>
                                </View>
                                {setting.type === "toggle" ? (
                                    <Switch value={setting.value}
                                        onValueChange={(value) => handleToggle(setting.id, value)}
                                        trackColor={{ false: "#2A2A2A", true: "#1DB954" }}
                                        thumbColor={"#FFFFFF"}
                                    />
                                ) : (
                                    <Ionicons name='chevron-forward' size={20} color={"#666"} />
                                )}
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>

                {/* Account Section */}
                <View className='px-6 pt-6'>
                    <Text className='text-text-primary font-bold text-lg mb-4'>Account</Text>
                    {accountSettings.map((setting) => (
                        <TouchableOpacity
                            key={setting.id}
                            className=' bg-surface rounded-2xl mb-3 p-4'
                            activeOpacity={0.7}
                        >
                            <View className='flex-row items-center'>
                                <View className='bg-primary/20 rounded-full justify-center
                                           w-12 h-12 items-center mr-4'>
                                    <Ionicons name={setting.icon as any} size={24} color={"#1DB954"} />
                                </View>
                                <View className='flex-1'>
                                    <Text
                                        className='text-text-primary text-base mb-1 font-bold'
                                    >{setting.title}</Text>
                                    <Text className='text-text-secondary text-sm '>{setting.description}</Text>
                                </View>

                                <Ionicons name='chevron-forward' size={20} color={"#666"} />
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
                {/* Delete Acc BTN */}
                <View className='px-6 pt4'>
                    <TouchableOpacity
                        className='bg-surface border-2 border-red-500/20
                      rounded-2xl p-5 flex-row items-center justify-between'
                        activeOpacity={0.7}
                    >
                        <View className='flex-row items-center'>
                                <View className='bg-primary/20 rounded-full justify-center
                                           w-12 h-12 items-center mr-4'>
                                    <Ionicons name="trash-outline" size={24} color={"#EF4444"} />
                                </View>
                                
                                 <View className=''>
                                    <Text className='text-red-500 font-bold text-balance mb-1'>
                                        Delete Account</Text>
                                    <Text className='text-text-secondary text-sm'>
                                        Delete Account Permanently</Text>
                                 </View>
                            </View>
                             <Ionicons name='chevron-forward' size={20} color={"#EF4444"} />
                    </TouchableOpacity>
                </View>

                {/* Information Alert */}

                <View className='px-6 pt-6 pb-4'>
                    <View className='bg-primary/10 rounded-2xl p-4 flex-row'>
                        <Ionicons name='information-circle-outline' size={24} color={"#1DB954"}/>
                        <Text className='text-text-secondary text-sm ml-3 flex-1'>
                            We take your privacy seriously. 
                            Your data is encripted and stored securely.
                            You can manage your privacy settings at any time
                        </Text>
                    </View>
                </View>

            </ScrollView>
        </SafeScreen>

    )
}

export default PrivacyAndSecurityScreen