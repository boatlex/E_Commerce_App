...
const AddressFormModal = ({
    ...
    // const [localZip, setLocalZip] = useState(addressForm.zipCode);

    // useEffect(() => {
    //      if (visible) {
    //         setLocalZip(addressForm.zipCode ? String(addressForm.zipCode) : "");
    //     }
    // }, [ visible]);


    // const handleZipChange = (text: string) => {
    //     const digits = text.replace(/[^0-9]/g, ''); // Remove non-numeric symbols
    //     setLocalZip(digits); // Instantly update input text field display
    //     onFormChange({ ...addressForm, zipCode: digits }); // Send data to parent
    // }
        ...
        >
            <SafeScreen>
                ...
                <View className='px-5 py-5 border-b border-surface flex-row items-center justify-between'>
                    ...
                    >{isEditing ? "Edit Address" : "Add New Address"}</Text>
                    ...
                    <View className='p-6 '>
                        ...
                        <View className='mb-5'>
                            ...
                            <TextInput
                                ...
                                placeholder='Street Adress apt/suite number'
                                placeholderTextColor={"#666"}
                        ...
                        {/*State Input */}
                        <View className='mb-5'>
                            <Text className='text-text-primary font-semibold mb-2'>Sate/Region</Text>
                            <TextInput
                                className='bg-surface text-text-primary px-4 py-4 rounded-2xl text-base'
                                value={addressForm.state}
                                placeholder='e.g., Ashanti, Greater-Accrea, Northern'
                                placeholderTextColor={"#666"}
                        ...
                        <View className='bg-surface p-4 rounded-2xl flex-row items-center justify-center mb-6'>
                            <Text className='text-text-primary font-semibold'>Set asdefault address</Text>
                            <Switch
                        ...
                        >
                            {isAddingAddress || isUpdatingAddress ? (
                                ...
                                >{isEditing ? "Save Changes" : " Add Address"}</Text>
                            )}
                        </TouchableOpacity>
                    </View>
                </ScrollView>
